...
-- * Description                                                                                                                                     *
-- ***************************************************************************************************************************************************
-- * 0.5.145/ 2015.07.23 / Llien: Added Export/Copy event option for Google Cal import
...
local function MaintenanceFrame(parent)
	...
	local eventCopyAll = UICreateFrame("RiftButton", frame:GetName() ..".EventCopyAll", eventFrame)
	...
	local function RefreshPermissions()
		...
		eventCopyAll:SetEnabled(permissions.assignID and true or false)
	...
	local function DisplayExportData( dataString )
		local exportContext = UI.CreateContext( addonID..'ExportFrame' )
		local exportBox = UI.CreateFrame( 'Frame',addonID..'ExportFrameWindow', exportContext )
		exportBox:SetPoint("BOTTOMLEFT", parent, "BOTTOMLEFT")
		exportBox:SetHeight(40)
		exportBox:SetWidth(400)
		exportBox:SetBackgroundColor(0,0,0,1)
		
		exportBox.label = UI.CreateFrame("Text", addonID.."ExportTextLabel", exportBox)
		exportBox.label:SetFontColor(1,1,1,1)
		exportBox.label:SetFontSize(20)
		exportBox.label:SetText('Copy to Export:')
		exportBox.label:ClearWidth()
		exportBox.label:SetPoint("CENTERLEFT", exportBox, "CENTERLEFT")
		exportBox.label:SetLayer(1)
		exportBox:SetWidth(550)
		exportBox.text = UI.CreateFrame("RiftTextfield", addonID.."ExportText", exportBox)
		exportBox.text:SetPoint("CENTERLEFT", exportBox.label, "CENTERRIGHT")
		exportBox.text:SetHeight(35)
		exportBox.text:SetWidth( 500 - exportBox.label:GetWidth())
		exportBox.text:SetText( dataString )	
		
		exportBox.text:SetBackgroundColor(.25,.25,.25,1)
		exportBox.text:SetLayer(1)
		
		exportBox.btClose = UI.CreateFrame("RiftButton",addonID.."ExportFrameClose", exportBox)
		exportBox.btClose:SetSkin("close")
		exportBox.btClose:SetPoint("CENTERRIGHT", exportBox, "CENTERRIGHT")
		exportBox.btClose:EventAttach(Event.UI.Button.Left.Press, function() exportBox:SetVisible(false) end, "Close")

	end
	
	local function OutputEventData()
		local outCSVTable = {}
		-- Header Row
		table.insert( outCSVTable, "Subject,Start date,Start time,End date, End time")
		for eventID, eventData in pairs(GetEventList() or {}) do
			local localEventStart = eventData.timestamp
			local startDate = ODate("%m/%d/%y", localEventStart)
			local startTime = ODate("%I:%M %p", localEventStart)	
			local localEventEnd
			if eventData.duration == 0 then
				localEventEnd = localEventStart + 86400
			else 
				local hours, minutes = MFloor(eventData.duration / 2), (eventData.duration % 2) * 30
				localEventEnd = eventData.timestamp + (hours*3600)+(minutes*60)
			end
			local endTime = ODate("%I:%M %p", localEventEnd)
			local endDate = ODate("%m/%d/%y", localEventEnd)
			local eventName = GetEventName(eventData.category, eventData.subcategory) or 'Unknown'
			table.insert( outCSVTable, eventName..','..startDate..','..startTime..','..endDate..','..endTime)
		end
		if (#outCSVTable == 1) then 
			print("No calendar data to export.")
		else 
			-- print("Exported "..(#outCSVTable-1).." event records.")
			local outputCSV = table.concat( outCSVTable, '\n')
			Internal.LastExport = outputCSV
			_G[addonID .. "LastExport"] = Internal.LastExport
			DisplayExportData( Internal.LastExport )
		end
	end
	...
	eventCopyAll:SetPoint("CENTERLEFT", eventDeleteAll, "CENTERRIGHT")
	eventCopyAll:SetText(L["ConfigMaintenance/EventsCopyAll"])
	...
	eventCopyAll:EventAttach(Event.UI.Button.Left.Press,
		function()
			OutputEventData()
		end, eventCopyAll:GetName() .. ".OnLeftPress")
